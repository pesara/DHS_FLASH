Ext.define('Base.components.MultiSortGrid', {
	...
	initComponent: function(args) {
		...
		this.normalGridConfig = {
			listeners: {
	            ...
	            beforerender: {
	                scope: this,
	                // wait for the first render to access the sortbar (we only want this once):
	                single: true,
	                // remove the extra toolbar on the locked portion of the grid.
	            	fn: function(grid) {
	            		this.addDocked(sortbar);
	                }
	            },
		        staterestore: {
		            scope: this,
    ...
    getSorters: function() {
        ...
        Ext.each(this.getDockedItems('toolbar[dock="top"]#sortbar')[0].query('button'), function(button) {
